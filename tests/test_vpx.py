...
class VP8PayloadDescriptorTest(TestCase):
    def test_no_picture_id(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x10")
        ...
        self.assertEqual(repr(descr), "VP8PayloadDescriptor(S=1, PID=0, pic_id=None)")
    ...
    def test_short_picture_id_17(self):
        ...
        From RFC 7741 - 4.6.3
        """
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x80\x11")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
        ...
        self.assertEqual(repr(descr), "VP8PayloadDescriptor(S=1, PID=0, pic_id=17)")

        self.assertEqual(rest, b"")

    def test_short_picture_id_127(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x80\x7f")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
    ...
    def test_long_picture_id_128(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x80\x80\x80")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
    ...
    def test_long_picture_id_4711(self):
        ...
        From RFC 7741 - 4.6.5
        """
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x80\x92\x67")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
    ...
    def test_tl0picidx(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\xc0\x92\x67\x81")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
        self.assertEqual(descr.picture_id, 4711)
    ...
    def test_tid(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x20\xe0")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
        self.assertEqual(descr.picture_id, None)
        self.assertEqual(descr.tl0picidx, None)
    ...
    def test_keyidx(self):
        descr, rest = VP8PayloadDescriptor.parse(b"\x90\x10\x1f")
        self.assertEqual(descr.partition_start, 1)
        self.assertEqual(descr.partition_id, 0)
        self.assertEqual(descr.picture_id, None)
        self.assertEqual(descr.tl0picidx, None)
...
class VP9PayloadDescriptorTest(TestCase):
    ...
    validTestPayload = b"\x8b\x8d\x86\x18\x05\x00\x02\x9c\x01\x14\x01"
    
    def test_picture_id(self):
        descr, rest = Vp9PayloadDescriptor.parse(self.validTestPayload)
        self.assertEqual(descr.start_of_layer_frame, 1)
        self.assertEqual(descr.end_of_layer_frame, 0)
        self.assertEqual(descr.picture_id, 3462)
        self.assertEqual(descr.spatial_layer_id, None)
        self.assertEqual(descr.temporal_layer_id, None)
        self.assertEqual(bytes(descr), b"\x8a\x8d\x86")
        self.assertEqual(repr(descr), "Vp9PayloadDescriptor(S=1, E=0, pic_id=3462, TLID=None, SLID=None)")

        self.assertEqual(rest, b"")
    
    def test_no_scalability_structure(self):
        descr, rest = Vp9PayloadDescriptor.parse(self.validTestPayload)
        self.assertEqual(descr.start_of_layer_frame, 1)
        self.assertEqual(descr.end_of_layer_frame, 0)
        self.assertEqual(descr.picture_id, 3462)
...
class Vp9Test(CodecTestCase):
    ...
    def test_encoder(self):
        ...
        self.assertIsInstance(encoder, Vp9Encoder)
